...
from avista_iot.config import Config
...
class IoTServer:
    ...
    def get_instance():
        ...
        """
        if IoTServer.__instance is None:
            IoTServer()
        return IoTServer.__instance

    def __init__(self):
        """ instantiate the app """
        if IoTServer.__instance is not None:
            raise Exception("This class is a singletion!")
        else:
            IoTServer.__instance = self
            self.proc = None
